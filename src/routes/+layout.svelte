<script>
	...
	function trackExpiredAt(endTime) {
		const interval = setInterval(() => {
			const currentTime = Date.now();
			const remainingTime = endTime - currentTime;
			if (remainingTime <= 0) {
				clearInterval(interval);
				window.location.href = data.MiniApp_URL;
			}
			console.log("Remaining expire seconds:", remainingTime);
		}, 1000);
	}
	...
	onMount(() => {
		...
		trackExpiredAt(data.x.expiredAt);
	});
...
>
	...
	>
		{#if mounted}
			...
			{:else}
				{#if !$state}
					<h1 class="pt-24 font-medium text-intro-title text-center">
						Connect
					</h1>
					<p
						class="text-base text-center font-medium text-light-text"
					>
				...
				{#if $state === "unsupported"}
					<h1 class="text-2xl font-medium mt-6 mb-3 text-center">
						Unsupported network
					</h1>
					...
					<Button class="mt-3 w-5/6 mx-auto" on:click={swicz}
	...
	{#if !$hideGradient}
		<div
			class="fixed max-w-[100vw] bottom-0 w-screen min-h-screen overflow-hidden pointer-events-none z-[-12] max-w-screen"
		>
			<img
